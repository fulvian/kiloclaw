...
export namespace Config {
  ...
  // Config override hierarchy: global > agency > env
  export interface OverrideHierarchy {
    global: Partial<ConfigInfo>
    agency: Record<string, Partial<ConfigInfo>>
    env: Partial<ConfigInfo>
  }
  ...
  export type ConfigInfo = z.infer<typeof ConfigInfo>
    ...
    for (const [key, value] of Object.entries(envVars)) {
      ...
      switch (configKey) {
        ...
        case "TELEMETRY_ENABLED":
          if (!result.telemetry) result.telemetry = {}
  ...
  // Create config loader with override hierarchy
  export const create = fn(
    z.object({
      global: ConfigInfo.partial().optional(),
      agency: z.record(z.string(), ConfigInfo.partial()).optional(),
    }),
    (input) => {
      ...
      if (input.global) {
        config = mergeDeep(config, input.global)
      }
