...
export namespace NativeMemoryAdapter {
  export function create(input?: {
    ...
    return {
      id: input?.id ?? "native.memory",
      capability: "memory_ops",
      probe: input?.probe ?? (async () => ({ healthy: true, latency_ms: 0, reason: "ok" })),
      invoke:
        input?.run ??
        (async () => ({ ok: false, error: "native.memory adapter requires injected runner", transient: false })),
    }
  }
}
