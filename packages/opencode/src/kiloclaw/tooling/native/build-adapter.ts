...
export namespace NativeBuildAdapter {
  export function create(input?: {
    ...
    return {
      id: input?.id ?? "native.build",
      capability: "build_test_ops",
      probe: input?.probe ?? (async () => ({ healthy: true, latency_ms: 0, reason: "ok" })),
      invoke:
        input?.run ??
        (async () => ({ ok: false, error: "native.build adapter requires injected runner", transient: false })),
    }
  }
}
