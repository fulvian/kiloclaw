      ...
      if (Flag.KILO_ROUTING_AGENCY_CONTEXT_ENABLED && agencyContext) {
        ...
        if (agencyContext.agencyId === "agency-knowledge") {
          ...
          agencyBlock = [
            ...
            "",
            "CRITICAL TOOL INSTRUCTIONS:",
            "- NBA Agency has specialized tools for live NBA data - ALWAYS use these instead of websearch",
            '- For COMPLEX analysis (betting, multiple games, full stats), use \'skill\' tool with {"name":"nba-analysis"} — it fetches games, stats, injuries, odds, and generates signals/recommendations in ONE call',
            "- For SPECIFIC data, use individual tools: nba-games, nba-stats, nba-injuries, nba-odds",
            "- Do NOT use websearch or webfetch for NBA data - the NBA tools provide official API data",
            ...
            "",
            "⚠️ CRITICAL: Call tools ONE AT A TIME. NEVER attempt parallel/multiple tool calls in a single response.",
            "Each response should contain EXACTLY ONE tool call. Wait for the result before making the next call.",
            "Do NOT use XML tags like <invoke> or <parameter> — use the standard tool calling format.",
            "",
            "NBA RECOMMENDED WORKFLOW:",
            "For betting analysis, the BEST approach is:",
            '  Step 1: Call skill tool with {"name":"nba-analysis"} → gets everything in one call',
            "  Step 2: If you need more detail, call individual tools one at a time",
            "",
            "If using individual tools, call them SEQUENTIALLY (one per response):",
            "  1. nba-games(date='YYYY-MM-DD') → get game IDs and team IDs",
            "  2. Wait for result, then nba-stats(teamIds=[...], type='team_stats')",
            "  3. Wait for result, then nba-stats(teamIds=[...], type='recent_games')",
            "  4. Wait for result, then nba-injuries(teamIds=[...])",
            "  5. Wait for result, then nba-odds(gameIds=[...])",
            "  6. Wait for result, then analyze and respond to user",
            "",
            "NBA Agency Tools (BallDontLie numeric IDs: team 1-30, game: numeric):",
            "- nba-games: Get today's games, scores, schedule (params: date?, teamIds?, status?)",
            "- nba-odds: Betting odds from Bet365/OddsAPI/Parlay (params: gameIds?, markets?, minEdge?)",
            "- nba-injuries: Injury reports with severity (params: teamIds?, status?)",
            "- nba-stats: Player/team stats, season averages, recent games (params: type, playerIds?, teamIds?, season?, lastNGames?)",
            "  - type=player_stats / player_season_averages / team_stats / recent_games",
            "- skill(nba-analysis): Complete analysis — games + stats + injuries + odds + signals + recommendations",
            "IMPORTANT: Only use official NBA Agency tools - never fabricate NBA statistics or odds.",
            "",
          ].join("\n")
          ...
          agencyBlock = [
            ...
            "",
            "CRITICAL TOOL INSTRUCTIONS:",
            "- For weather queries: use the 'skill' tool with mode='execute' and name='weather-current', 'weather-forecast', or 'weather-alerts'",
            "- Example: skill { name: 'weather-forecast', mode: 'execute' }",
            "- DO NOT use websearch or webfetch for weather data",
            ...
            "",
            "WEATHER SKILLS (use skill tool with mode='execute'):",
            "- For current conditions: skill { name: 'weather-current', mode: 'execute' }",
            "- For forecasts: skill { name: 'weather-forecast', mode: 'execute' }",
            "- For alerts: skill { name: 'weather-alerts', mode: 'execute' }",
            "",
            "PROVIDER PRIORITY:",
