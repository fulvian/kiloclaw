      ...
      if (Flag.KILO_ROUTING_AGENCY_CONTEXT_ENABLED && agencyContext) {
        ...
        if (agencyContext.agencyId === "agency-knowledge") {
          ...
          agencyBlock = [
            ...
            '- Use \'skill\' tool with {"name":"nba-analysis"} for full betting analysis with signals and recommendations',
            "- Use 'nba-games' tool first to get today's games — it returns game IDs and team IDs needed by other tools",
            "- Use 'nba-odds' tool for betting odds from multiple bookmakers (Bet365, Odds API, Parlay)",
            "- Use 'nba-injuries' tool for injury reports (BallDontLie + ESPN)",
            "- Use 'nba-stats' tool for player stats, team stats, season averages, and recent game results",
            "- Do NOT use websearch or webfetch for NBA data - the NBA tools provide official API data",
            "- 'nba-analysis' skill orchestrates all tools internally - prefer it for complete analysis",
            ...
            "",
            "NBA TOOL CHAINING (follow this workflow):",
            "1. Call nba-games(date='YYYY-MM-DD') → get game IDs and team IDs",
            "2. Call nba-stats(teamIds=[...], type='team_stats') → team statistics",
            "3. Call nba-stats(teamIds=[...], type='recent_games') → last 5 games",
            "4. Call nba-injuries(teamIds=[...]) → injury reports",
            "5. Call nba-odds(gameIds=[...]) → betting odds",
            "6. Call nba-stats(teamIds=[...], type='player_season_averages') → top player stats",
            "All IDs are BallDontLie numeric format (team IDs: 1-30, game IDs: numeric).",
            "",
            "NBA Agency Tools:",
            "- nba-games: Get today's NBA games, live scores, schedule (params: date?, teamIds?, status?)",
            "- nba-odds: Get betting odds from Bet365/OddsAPI/Parlay (params: gameIds?, markets?, minEdge?)",
            "- nba-injuries: Get injury reports with severity (params: teamIds?, status?)",
            "- nba-stats: Player stats, team stats, season averages, recent games (params: type, playerIds?, teamIds?, season?, lastNGames?)",
            "  - type=player_stats: Per-game player box score stats",
            "  - type=player_season_averages: Season averages for players",
            "  - type=team_stats: Team season statistics",
            "  - type=recent_games: Last N games for a team",
            "- nba-analysis skill: Full analysis with signals, value detection, and recommendations",
