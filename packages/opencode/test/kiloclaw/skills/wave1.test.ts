...
import { SkillId } from "@/kiloclaw/types"
import type { Skill } from "@/kiloclaw/skill"

// Result type interfaces for skill execute() return types
...
interface ComparisonResult {
  diff: Array<{ type: "addition" | "deletion" | "modification"; content: string }>
  summary: { additions: number; deletions: number; modifications: number }
  conflicts: Array<{ before: string; after: string }>
}

interface DocumentAnalysisResult {
  sections: Array<{ title?: string; content: string }>
  summary: string
  metadata?: { hasCodeBlocks: boolean; wordCount: number }
}

interface SimplificationResult {
  metrics: { cyclomaticComplexity: number; cognitiveComplexity: number; linesOfCode: number }
  suggestions: string[]
  simplified: string
}
...
interface SynthesisResult {
  ...
  insights: string[]
  keyThemes?: string[]
  sourceReferences: Array<{ id: string; title: string }>
  confidence: number
}
...
describe("WP4.3 Wave 1: Development Agency Skills", () => {
  describe("code-review skill", () => {
    it("should have correct metadata", () => {
      expect(CodeReviewSkill.id as string).toBe("code-review")
    ...
    it("should detect var keyword issues", async () => {
      ...
      expect(result.issues.some((i: { rule: string }) => i.rule === "no-var")).toBe(true)
    })
  })

  describe("debugging skill", () => {
    it("should have correct metadata", () => {
      expect(DebuggingSkill.id as string).toBe("debugging")
    ...
    it("should handle empty code", async () => {
      const result = (await DebuggingSkill.execute({ code: "", error: "" }, SKILL_CONTEXT)) as DebuggingResult
  ...
  describe("tdd skill", () => {
    it("should have correct metadata", () => {
      expect(TddSkill.id as string).toBe("tdd")
    ...
    it("should handle different frameworks", async () => {
      const jestResult = (await TddSkill.execute(
        { code: "const fn = () => {}", framework: "jest" },
        SKILL_CONTEXT,
      )) as TddResult
      const mochaResult = (await TddSkill.execute(
        { code: "const fn = () => {}", framework: "mocha" },
        SKILL_CONTEXT,
      )) as TddResult
    ...
    it("should handle empty code", async () => {
      const result = (await TddSkill.execute({ code: "", framework: "jest" }, SKILL_CONTEXT)) as TddResult
  ...
  describe("comparison skill", () => {
    it("should have correct metadata", () => {
      expect(ComparisonSkill.id as string).toBe("comparison")
    ...
    it("should detect modifications", async () => {
      const result = (await ComparisonSkill.execute(
        { before: "const a = 1;", after: "const a = 2;" },
        SKILL_CONTEXT,
      )) as ComparisonResult
    ...
    it("should handle empty inputs", async () => {
      const result = (await ComparisonSkill.execute({ before: "", after: "" }, SKILL_CONTEXT)) as ComparisonResult
  ...
  describe("document-analysis skill", () => {
    it("should have correct metadata", () => {
      expect(DocumentAnalysisSkill.id as string).toBe("document-analysis")
  ...
  describe("simplification skill", () => {
    it("should have correct metadata", () => {
      expect(SimplificationSkill.id as string).toBe("simplification")
    ...
    it("should detect high complexity", async () => {
      ...
      expect(result.suggestions.some((s: string) => s.includes("complexity"))).toBe(true)
    })

    it("should suggest improvements", async () => {
      const result = (await SimplificationSkill.execute({ code: "var x = 1;" }, SKILL_CONTEXT)) as SimplificationResult
      expect(result.suggestions.length).toBeGreaterThan(0)
      expect(result.suggestions.some((s: string) => s.includes("const"))).toBe(true)
    })

    it("should handle empty code", async () => {
      const result = (await SimplificationSkill.execute({ code: "" }, SKILL_CONTEXT)) as SimplificationResult
...
describe("WP4.3 Wave 1: Knowledge Agency Skills", () => {
  describe("web-research skill", () => {
    it("should have correct metadata", () => {
      expect(WebResearchSkill.id as string).toBe("web-research")
    ...
    it("should perform web search", async () => {
      const result = (await WebResearchSkill.execute(
        { query: "typescript best practices", sources: 3 },
        SKILL_CONTEXT,
    ...
    it("should handle empty query", async () => {
      const result = (await WebResearchSkill.execute({ query: "", sources: 5 }, SKILL_CONTEXT)) as WebResearchResult
  ...
  describe("literature-review skill", () => {
    it("should have correct metadata", () => {
      expect(LiteratureReviewSkill.id as string).toBe("literature-review")
    ...
    it("should search academic papers", async () => {
      const result = (await LiteratureReviewSkill.execute(
        { topic: "machine learning", count: 3 },
        SKILL_CONTEXT,
      )) as LiteratureReviewResult
  ...
  describe("fact-check skill", () => {
    it("should have correct metadata", () => {
      expect(FactCheckSkill.id as string).toBe("fact-check")
    ...
    it("should verify known true claim", async () => {
      const result = (await FactCheckSkill.execute({ claim: "The earth is round" }, SKILL_CONTEXT)) as FactCheckResult
    ...
    it("should verify known false claim", async () => {
      const result = (await FactCheckSkill.execute({ claim: "The earth is flat" }, SKILL_CONTEXT)) as FactCheckResult
    ...
    it("should handle empty claim", async () => {
      const result = (await FactCheckSkill.execute({ claim: "" }, SKILL_CONTEXT)) as FactCheckResult
  ...
  describe("synthesis skill", () => {
    it("should have correct metadata", () => {
      expect(SynthesisSkill.id as string).toBe("synthesis")
    ...
    it("should handle empty documents", async () => {
      const result = (await SynthesisSkill.execute({ documents: [] }, SKILL_CONTEXT)) as SynthesisResult
      expect(result.confidence).toBe(0)
  ...
  describe("critical-analysis skill", () => {
    it("should have correct metadata", () => {
      expect(CriticalAnalysisSkill.id as string).toBe("critical-analysis")
    ...
    it("should generate counter-arguments", async () => {
      const result = (await CriticalAnalysisSkill.execute(
        { claim: "All programmers should learn Rust" },
        SKILL_CONTEXT,
      )) as CriticalAnalysisResult
    ...
    it("should handle empty claim", async () => {
      const result = (await CriticalAnalysisSkill.execute({ claim: "" }, SKILL_CONTEXT)) as CriticalAnalysisResult
...
describe("WP4.3 Wave 1: Skill Registry Integration", () => {
  it("should export 15 development skills", () => {
    expect(DEVELOPMENT_SKILL_COUNT).toBe(15)
  ...
  it("should have unique skill IDs", () => {
    const ids = allWave1Skills.map((s: Skill) => s.id)
