 ...
 * then copies it into packages/kilo-vscode/bin/kiloclaw.
 *
...
const targetBinPath = join(targetBinDir, "kiloclaw")
...
function sourceBinaryPath(): string {
  return join(opencodeDir, "dist", `@kilocode/cli-${process.platform}-${process.arch}`, "bin", "kiloclaw")
}
...
async function rebuild() {
  ...
  try {
    ...
    log(`Binary updated (${elapsed}s): ${relative(packagesDir, source)} -> bin/kiloclaw`)
  } catch (err) {
